...
export class RideRequestService {
  constructor(
    /**
     * Repository used to create and save RideRequest records.
     */
    @InjectRepository(RideRequest)
    private readonly rideRequestRepository: Repository<RideRequest>,

    /**
     * UserService allows us to find the authenticated
     * passenger using the ID obtained from the JWT.
     */
  ...
  async create(
    ...
    /**
     * If the user no longer exists, we cannot create
     * a ride request for them.
     */
    if (!passenger) {
